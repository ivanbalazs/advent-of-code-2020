...
const list = `13-14 f: ffffffffnfffvv
...
const part1 = list.reduce((num, i) => {
    const x = i[3].replace(new RegExp(`[^${i[2]}]`, 'g'), '');
    return num + +(x.length >= i[0] && x.length <= i[1]);
}, 0);

console.log('Part 1', part1);

const part2 = list.reduce((num, i) => {
    const str = ' ' + i[3];
    return num + +(str[i[0]] === i[2] ^ str[i[1]] === i[2]);
}, 0);
